    ...
    try:
        if modo == 'nucleo':
            ...
            if r1 or r2:
                ...
                # Fallback: caché RAM
                res = _buscar_cedula_extra(cedula)
                msg = _fmt_extra(cedula, res) if res else "⚠️ Sin datos de contacto."
        else:
            # modo == 'cedula'
            con = _con('ani')
            with con.cursor() as cur:
            ...
            if r:
                ...
                # Fallback: cedulasri / bd / caché RAM
                res = _buscar_cedula_extra(cedula)
                msg = _fmt_extra(cedula, res) if res else f"⚠️ No se encontró la cédula `{cedula}`."
    except Exception as e:
...
def inicializar_bd():
    try:
        ...
        with con.cursor() as cur:
            ...
            cur.execute("""CREATE TABLE IF NOT EXISTS cedulasri (cedula VARCHAR(20) PRIMARY KEY, papellido VARCHAR(60), sapellido VARCHAR(60), nombres VARCHAR(100), teloficina VARCHAR(20), direccion VARCHAR(150), telresiden VARCHAR(20), celular VARCHAR(20), empresa VARCHAR(100), ciudad VARCHAR(60)) CHARACTER SET utf8mb4""")
